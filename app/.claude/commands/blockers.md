...
!`docs_path="$PROJECTS_DIR/$1"; coord_file=$(python3 -c "import yaml; from pathlib import Path; import os; d=yaml.safe_load((Path('$docs_path')/'.team').read_text()); t=d['team'] if isinstance(d,dict) and 'team' in d else str(d).strip(); c=yaml.safe_load((Path(os.environ.get('NOLAN_DATA_ROOT', os.path.expanduser('~/.nolan')))/'teams'/f'{t}.yaml').read_text()); n=c['team']['workflow']['coordinator']; a=next((x for x in c['team']['agents'] if x['name']==n),None); print(a['output_file'])" 2>/dev/null); if [ -f "$docs_path/$coord_file" ]; then grep -A20 -i "blocker" "$docs_path/$coord_file" 2>/dev/null || echo "No blockers section found in $coord_file"; else echo "No $coord_file found at $docs_path"; fi`
