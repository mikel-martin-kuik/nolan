...
!`agent="${AGENT_NAME:-}"; if [[ "$agent" == "ralph" ]] || [[ "$agent" =~ ^ralph- ]]; then exit 0; fi; coord=$(python3 -c "import yaml, os; from pathlib import Path; data_root=Path(os.environ.get('NOLAN_DATA_ROOT', os.path.expanduser('~/.nolan'))); cp=next((data_root/'teams').rglob((os.environ.get('TEAM_NAME') or 'default')+'.yaml'),None); c=yaml.safe_load(cp.read_text()) if cp else exit(1); print(c['team']['workflow']['coordinator'])" 2>/dev/null || echo "dan"); if [ "$agent" != "$coord" ]; then echo "ERROR: This command is restricted."; exit 1; fi`
