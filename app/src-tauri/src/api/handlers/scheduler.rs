//! Scheduler (scheduled agents) HTTP handlers
...
use crate::scheduler::types::ScheduledAgentConfig;
...
pub async fn list_agents() -> Result<Json<serde_json::Value>, impl IntoResponse> {
    match commands::list_scheduled_agents().await {
...
) -> Result<Json<ScheduledAgentConfig>, impl IntoResponse> {
    match commands::get_scheduled_agent(name).await {
...
pub async fn create_agent(
    ...
    match commands::create_scheduled_agent(req.config).await {
...
pub async fn update_agent(
    ...
    match commands::update_scheduled_agent(name, req.config).await {
        Ok(_) => Ok(Json(serde_json::json!({ "success": true }))),
        Err(e) => Err(error_response(StatusCode::BAD_REQUEST, e)),
    }
}
...
pub async fn delete_agent(
    ...
    match commands::delete_scheduled_agent(name).await {
        Ok(_) => Ok(Json(serde_json::json!({ "success": true }))),
        Err(e) => Err(error_response(StatusCode::BAD_REQUEST, e)),
    }
}
...
pub async fn toggle_agent(
    ...
    match commands::toggle_scheduled_agent(name, req.enabled).await {
        Ok(_) => Ok(Json(serde_json::json!({ "success": true }))),
        Err(e) => Err(error_response(StatusCode::BAD_REQUEST, e)),
    }
}
...
pub async fn test_agent(
    ...
    match commands::test_scheduled_agent(name).await {
...
pub async fn trigger_agent(
    ...
    match commands::trigger_scheduled_agent_api(name).await {
...
pub async fn get_run_history(
    ...
    match commands::get_scheduled_run_history(Some(name), None).await {
...
/// Get run history for all scheduled agents
pub async fn get_all_run_history(
    Query(query): Query<AllHistoryQuery>,
) -> Result<Json<serde_json::Value>, impl IntoResponse> {
    match commands::get_scheduled_run_history(None, query.limit).await {
        Ok(history) => Ok(Json(serde_json::json!(history))),
        Err(e) => Err(error_response(StatusCode::INTERNAL_SERVER_ERROR, e)),
    }
}
...
pub async fn get_run_log(
    ...
    match commands::get_scheduled_run_log(run_id).await {
...
pub async fn relaunch_session(
    ...
    match commands::relaunch_scheduled_session_api(run_id, req.follow_up_prompt).await {
        Ok(result) => Ok(Json(serde_json::json!({ "result": result }))),
        Err(e) => Err(error_response(StatusCode::BAD_REQUEST, e)),
    }
}
...
pub async fn read_claude_md(Path(name): Path<String>) -> Result<Json<String>, impl IntoResponse> {
    match commands::read_scheduled_agent_claude_md(name).await {
...
pub async fn write_claude_md(
    ...
    match commands::write_scheduled_agent_claude_md(name, req.content).await {
        Ok(_) => Ok(Json(serde_json::json!({ "success": true }))),
        Err(e) => Err(error_response(StatusCode::BAD_REQUEST, e)),
    }
}
...
pub async fn init() -> Result<Json<serde_json::Value>, impl IntoResponse> {
    match commands::init_scheduler().await {
        Ok(_) => Ok(Json(serde_json::json!({ "success": true }))),
        Err(e) => Err(error_response(StatusCode::INTERNAL_SERVER_ERROR, e)),
    }
}
...
pub async fn shutdown() -> Result<Json<serde_json::Value>, impl IntoResponse> {
    match commands::shutdown_scheduler().await {
        Ok(_) => Ok(Json(serde_json::json!({ "success": true }))),
        Err(e) => Err(error_response(StatusCode::INTERNAL_SERVER_ERROR, e)),
    }
}
...
pub async fn cancel_agent(
    ...
    match commands::cancel_scheduled_agent(name).await {
        Ok(_) => Ok(Json(serde_json::json!({ "success": true }))),
        Err(e) => Err(error_response(StatusCode::BAD_REQUEST, e)),
    }
}
...
pub async fn get_health() -> Result<Json<serde_json::Value>, impl IntoResponse> {
    match commands::get_scheduler_health().await {
...
pub async fn get_group(
    ...
    match commands::get_scheduled_group(group_id).await {
...
pub async fn create_group(
    ...
    match commands::create_scheduled_group(req.group).await {
        Ok(_) => Ok(Json(serde_json::json!({ "success": true }))),
        Err(e) => Err(error_response(StatusCode::BAD_REQUEST, e)),
    }
}
...
pub async fn update_group(
    ...
    match commands::update_scheduled_group(req.group).await {
        Ok(_) => Ok(Json(serde_json::json!({ "success": true }))),
        Err(e) => Err(error_response(StatusCode::BAD_REQUEST, e)),
    }
}
...
pub async fn delete_group(
    ...
    match commands::delete_scheduled_group(group_id).await {
        Ok(_) => Ok(Json(serde_json::json!({ "success": true }))),
        Err(e) => Err(error_response(StatusCode::BAD_REQUEST, e)),
    }
}
