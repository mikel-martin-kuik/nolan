...
impl TeamPipelineManager {
    ...
    pub fn find_pipeline_by_project(&self, project_name: &str) -> Result<Option<TeamPipeline>, String> {
    ...
    pub fn list_pipelines(&self, status_filter: Option<PipelineStatus>) -> Result<Vec<TeamPipeline>, String> {
    ...
    pub fn get_next_action(&self, pipeline: &TeamPipeline) -> Option<TeamPipelineNextAction> {
        ...
        let current_stage = pipeline.stages.iter()
            .find(|s| s.phase_name == pipeline.current_phase &&
                      s.stage_type == pipeline.current_stage_type)?;

        match (&current_stage.stage_type, &current_stage.status) {
            // Phase execution completed successfully - trigger validator
            (TeamPipelineStageType::PhaseExecution, PipelineStageStatus::Success) => {
                current_stage.output_file.as_ref().map(|output_file| {
            ...
            (TeamPipelineStageType::PhaseValidation, PipelineStageStatus::Pending) => {
                ...
                let exec_stage = pipeline.stages.iter()
                    .find(|s| s.phase_name == current_stage.phase_name &&
                    ...
                    match verdict.verdict {
                        ...
                        PhaseVerdictType::Revision => {
                            ...
                            Some(TeamPipelineNextAction::RetryPhase {
                                ...
                                prompt: verdict.revision_prompt.clone()
                        ...
                        PhaseVerdictType::Failed => {
                            Some(TeamPipelineNextAction::EscalateToHuman {
                                phase_name: current_stage.phase_name.clone(),
    ...
    fn save_pipeline(&self, pipeline: &TeamPipeline) -> Result<(), String> {
        ...
        fs::write(&path, content)
            .map_err(|e| format!("Failed to write team pipeline file: {}", e))
    }
    ...
    fn find_next_phase(&self, pipeline: &TeamPipeline, current_phase: &str) -> Option<(String, String)> {
...
mod tests {
    ...
    use crate::config::{TeamConfig, Team, WorkflowConfig, PhaseConfig};
                ...
                workflow: WorkflowConfig {
                    ...
                    phases: vec![
                        ...
                        PhaseConfig {
                            ...
                            template: None,
                            next: Some("Implementation".to_string()),
                            on_reject: Some("Research".to_string()),
                        },
                    ],
                },
            },
    ...
    fn test_update_stage() {
        ...
        let exec_stage = updated.stages.iter()
            .find(|s| s.phase_name == "Research" &&
                      s.stage_type == TeamPipelineStageType::PhaseExecution)
            .unwrap();
