...
pub fn extract_cost_from_log_file(log_path: &str) -> Option<f32> {
    ...
    for line in content.lines().rev() {
        // Search from end (result is last)
...
fn find_agent_dir(name: &str) -> Result<PathBuf, String> {
    ...
    Err(format!("Agent directory '{}' not found", name))
}

// CancellationToken is now defined in types.rs

/// Output event sender for real-time streaming
pub type OutputSender = broadcast::Sender<ScheduledOutputEvent>;

/// Extra environment variables to pass to scheduled agents
...
pub async fn execute_cron_agent_with_env(
    ...
    if !config.concurrency.allow_parallel && manager.is_running(&config.name).await {
        ...
        if let Some(ref sender) = output_sender {
            let _ = sender.send(ScheduledOutputEvent {
                run_id: run_id.clone(),
                agent_name: config.name.clone(),
    ...
    loop {
        ...
        match &result {
            Ok(log)
                if log.status == ScheduledRunStatus::Failed
            ...
            _ => {
                ...
                if let Ok(ref log) = result {
                    let _ = manager
                        .update_agent_state(&config.name, Some(log.status.clone()), None)
                        .await;
                }
                return result;
            }
...
async fn execute_single_run(
    ...
    let scheduler_runs_dir = paths::get_scheduler_runs_dir()?;
    let runs_dir = scheduler_runs_dir.join(&date_str);
    ...
    // Generate tmux session name (format: agentname-runid)
    let session_name = format!("{}-{}", config.name, &run_id);

    // Generate Claude session ID for --resume capability
    let claude_session_id = Uuid::new_v4().to_string();

    // Create or use cancellation token (needed for cancel_scheduled_agent to signal cancellation)
    ...
    if let Some(ref sender) = output_sender {
        let _ = sender.send(ScheduledOutputEvent {
            ...
            event_type: OutputEventType::Status,
            content: format!(
                "Starting {} in tmux session {} (attempt {})",
                config.name, session_name, attempt
            ),
    ...
    let agent_dir = find_agent_dir(&config.name)?;
            ...
            if let Some(git_root) = worktree::detect_git_root(&repo_path) {
                ...
                match worktree::create_worktree(
                    ...
                    Ok(base_commit) => {
                        ...
                        (
                            worktree_path.clone(),
                            Some((worktree_path, branch_name, base_commit)),
                        )
                    }
                    Err(e) => {
                        // Log warning but continue without worktree
                        eprintln!(
                            "[Scheduler] Warning: Failed to create worktree for {}: {}",
                            config.name, e
                        );
                        if let Some(ref sender) = output_sender {
                            let _ = sender.send(ScheduledOutputEvent {
                                ...
                                event_type: OutputEventType::Status,
                                content: format!(
                                    "Warning: Running without worktree isolation: {}",
                                    e
                                ),
                                timestamp: Utc::now().to_rfc3339(),
                            });
    ...
    spawn_env.insert(
        "NOLAN_ROOT".to_string(),
        nolan_root.to_string_lossy().to_string(),
    );
    spawn_env.insert(
        "NOLAN_DATA_ROOT".to_string(),
        nolan_data_root.to_string_lossy().to_string(),
    );
    ...
    let (wt_path, wt_branch, wt_commit) = match &worktree_info {
        ...
        None => {
            ...
            let inherited_path = extra_env
                .as_ref()
                .and_then(|e| e.get("ANALYZED_WORKTREE_PATH").cloned());
            let inherited_branch = extra_env
                .as_ref()
                .and_then(|e| e.get("ANALYZED_WORKTREE_BRANCH").cloned());
            let inherited_commit = extra_env
                .as_ref()
                .and_then(|e| e.get("ANALYZED_BASE_COMMIT").cloned());
    ...
    let initial_log = ScheduledRunLog {
        ...
        analyzer_verdict: None,
        pipeline_id: None,
        parent_run_id: extra_env
            .as_ref()
            .and_then(|e| e.get("ANALYZED_RUN_ID").cloned()),
    };
    ...
    // Register as running in manager (with cancellation token for cancel_scheduled_agent)
    manager
        .register_running_with_worktree(
            &run_id,
            &config.name,
    ...
    let (status, exit_code, error) = match result {
        ...
        TmuxWaitResult::Timeout => {
            ...
            let _ = std::process::Command::new("tmux")
                ...
                .output();
            (
                ScheduledRunStatus::Timeout,
                None,
                Some(format!("Timeout after {}s", config.timeout)),
            )
        }
    ...
    let total_cost_usd = cli_result
        .total_cost_usd
        .or_else(|| extract_cost_from_log(&log_file));

    let run_log = ScheduledRunLog {
        run_id: run_id.clone(),
        agent_name: config.name.clone(),
        started_at: started_at.to_rfc3339(),
    ...
    std::fs::write(&json_file, json).map_err(|e| format!("Failed to write run log: {}", e))?;
...
pub async fn cancel_scheduled_agent(manager: &SchedulerManager, agent_name: &str) -> Result<(), String> {
    let process = manager
        .get_running_process(agent_name)
        .await
...
pub async fn execute_cron_agent_simple(
    ...
    let nolan_root = paths::get_nolan_root()?;
    let scheduler_runs_dir = paths::get_scheduler_runs_dir()?; // Run logs (data)
    let runs_dir = scheduler_runs_dir.join(&date_str);
    std::fs::create_dir_all(&runs_dir)
        .map_err(|e| format!("Failed to create runs directory: {}", e))?;
    ...
    let (status, exit_code, error) = match result {
        Ok(Ok(exit)) => {
            if exit.success() {
                (ScheduledRunStatus::Success, exit.code(), None)
            } else {
                ...
                (ScheduledRunStatus::Failed, exit.code(), err_msg)
            }
        }
        Ok(Err(e)) => (ScheduledRunStatus::Failed, None, Some(e.to_string())),
        Err(_) => {
...
pub fn get_analyzer_trigger_info(
    ...
    let should_trigger = match run_log.status {
        ScheduledRunStatus::Success => analyzer_config.on_success,
        ScheduledRunStatus::Failed => analyzer_config.on_failure,
        ScheduledRunStatus::Timeout => analyzer_config.on_timeout,
        _ => false,
    };
    ...
    env_vars.insert(
        "ANALYZED_OUTPUT_FILE".to_string(),
        run_log.output_file.clone(),
    );
    env_vars.insert(
        "ANALYZED_STATUS".to_string(),
        format!("{:?}", run_log.status).to_lowercase(),
    );
