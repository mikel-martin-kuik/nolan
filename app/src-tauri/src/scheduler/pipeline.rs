...
use crate::scheduler::types::{
    AgentRole, AnalyzerVerdict, AnalyzerVerdictType, ScheduledRunLog, ScheduledRunStatus, Pipeline,
    PipelineDefinition, PipelineEvent, PipelineEventType, PipelineInputs, PipelineNextAction,
    PipelineStage, PipelineStageStatus, PipelineStageType, PipelineStatus,
};
...
impl PipelineManager {
    ...
    pub fn get_pipeline(&self, id: &str) -> Result<Pipeline, String> {
        ...
        serde_json::from_str(&content).map_err(|e| format!("Failed to parse pipeline JSON: {}", e))
    }
            ...
            if let Ok(entries) = fs::read_dir(&dir_path) {
                for entry in entries.flatten() {
                    ...
                    if path.extension().map_or(false, |ext| ext == "json") {
                        ...
                        if name.starts_with("idea-implementer-") {
                            if let Ok(content) = fs::read_to_string(&path) {
                                if let Ok(run_log) = serde_json::from_str::<ScheduledRunLog>(&content) {
                                    if run_log.status == ScheduledRunStatus::Running {
    ...
    /// Convert a running implementer ScheduledRunLog to a live Pipeline
    fn run_log_to_pipeline(&self, run_log: &ScheduledRunLog) -> Pipeline {
        // Generate a pipeline ID from the run (or use existing if set)
        let pipeline_id = run_log
            .pipeline_id
            .clone()
            .unwrap_or_else(|| format!("live-{}", run_log.run_id));

        // Extract idea info from run label or run_id
        let idea_title = run_log
            .label
            .clone()
    ...
    pub fn find_pipeline_by_worktree(
        &self,
        worktree_branch: &str,
    ...
    pub fn get_next_action(&self, pipeline: &Pipeline) -> Option<PipelineNextAction> {
        ...
        match (&current_stage.stage_type, &current_stage.status) {
            ...
            (PipelineStageType::Implementer, PipelineStageStatus::Success) => current_stage
                .run_id
                .as_ref()
                .map(|run_id| PipelineNextAction::TriggerAnalyzer {
                    run_id: run_id.clone(),
                }),
                        ...
                        AnalyzerVerdictType::Followup => {
                            ...
                            impl_stage.run_id.as_ref().map(|run_id| {
                                PipelineNextAction::RelaunchSession {
                                    run_id: run_id.clone(),
                                    prompt: verdict.follow_up_prompt.clone().unwrap_or_else(|| {
                                        "Continue the implementation.".to_string()
                                    }),
                                }
                            })
                        }
                        AnalyzerVerdictType::Failed => Some(PipelineNextAction::Fail {
    ...
    fn save_pipeline(&self, pipeline: &Pipeline) -> Result<(), String> {
        ...
        fs::write(&path, content).map_err(|e| format!("Failed to write pipeline file: {}", e))
    }
        ...
        if has_running {
            ...
            if let Some(idx) = failed_idx {
                ...
                if pipeline
                    ...
                    .iter()
                    .skip(idx + 1)
                    .any(|s| s.status == PipelineStageStatus::Pending)
                {
    ...
    fn test_load_pipeline_definition() {
        ...
        if data_root.exists() {
            ...
            if let Ok(def) = result {
                ...
                println!(
                    "Stages: {:?}",
                    def.stages.iter().map(|s| &s.name).collect::<Vec<_>>()
                );
            } else {
