...
export const CronAgentDetailPage: React.FC<CronAgentDetailPageProps> = ({
  ...
  const [agent, setAgent] = useState<ScheduledAgentInfo | null>(null);
  const [config, setConfig] = useState<ScheduledAgentConfig | null>(null);
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; run: ScheduledRunLog } | null>(null);
  ...
  const fetchAgent = useCallback(async () => {
    try {
      const agents = await invoke<ScheduledAgentInfo[]>('list_scheduled_agents');
  ...
  const fetchInstructions = useCallback(async () => {
    try {
      const result = await invoke<string | { content: string }>('read_scheduled_agent_claude_md', { name: agentName });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      await Promise.all([
        fetchAgent().catch(() => {}),
        invoke<ScheduledAgentConfig>('get_scheduled_agent', { name: agentName })
  ...
  const handleSaveConfig = async () => {
    ...
    setSaving(true);
    try {
      await invoke('update_scheduled_agent', { name: agentName, config });
  ...
  const handleSaveInstructions = async () => {
    setSaving(true);
    try {
      await invoke('write_scheduled_agent_claude_md', { name: agentName, content: instructionsContent });
  ...
  const updateConfig = (updates: Partial<ScheduledAgentConfig>) => {
    if (!config) return;
  ...
  const handleRelaunch = async () => {
    ...
    setRelaunching(true);
    try {
      await invoke('relaunch_scheduled_session', {
  ...
  const canRelaunch = (run: ScheduledRunLog) => {
    // Can relaunch if:
  ...
  const handleRunContextMenu = (e: React.MouseEvent, run: ScheduledRunLog) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const getVerdictBadge = (run: ScheduledRunLog) => {
  ...
  const getRelaunchPrompt = (run: ScheduledRunLog) => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* Relaunch Modal */}
      <Dialog open={!!relaunchRunId} onOpenChange={(open) => {
        if (!open) {
          setRelaunchRunId(null);
          setRelaunchPrompt('');
        }
