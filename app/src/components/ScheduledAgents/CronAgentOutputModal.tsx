...
import { Square, Loader2, CheckCircle, Cpu, Terminal, MessageSquare } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from '@/components/ui/dialog';
import { useToastStore } from '@/store/toastStore';
import type { ScheduledAgentInfo, ScheduledOutputEvent } from '@/types';
    ...
    const poll = setInterval(async () => {
      ...
      if (runId) {
        try {
          ...
          if (logContent) {
            const events: ScheduledOutputEvent[] = logContent.split('\n').filter(Boolean).map(line => ({
      ...
      <DialogContent className="max-w-4xl max-h-[80vh] flex flex-col p-0 gap-0">
        ...
        <DialogHeader className="flex flex-row items-center justify-between p-4 border-b border-border space-y-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle>Output - {agentName}</DialogTitle>
              <DialogDescription>
          ...
          {isRunning && (
            <Button variant="destructive" size="sm" onClick={() => agentName && onCancel(agentName)} className="mr-8">
