...
NOLAN_ROOT="$(dirname "$SCRIPT_DIR")"
APP_DIR="$NOLAN_ROOT/app"
...
start_ttyd() {
    ...
    if ! command -v ttyd &> /dev/null; then
        echo -e "  ${YELLOW}⚠ ttyd not installed (run scripts/setup.sh to install)${NC}"
        return 0
    fi
    ...
    if command -v ss &> /dev/null && ss -tln | grep -q ":$TTYD_PORT "; then
        ...
        echo "    Try: NOLAN_TTYD_PORT=7682 ./scripts/start.sh"
        return 0
    fi
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --help|-h)
                ...
                echo ""
                echo "Examples:"
                echo "  ./scripts/start.sh              # Launch production build"
                echo "  ./scripts/start.sh --dev        # Launch dev mode with hot reload"
                echo ""
                echo "Environment:"
